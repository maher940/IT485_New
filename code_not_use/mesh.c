...
Mesh * mesh_load_from_obj(char * filename)
{
	char buf[512];
	Mesh *mesh = NULL;
	int numvertices = 0, numnormals = 0, numfaces = 0, numtexels =0;
	int v, t, n, f;
	if (!filename)
	{
	...
	if (!file)
	{
		slog("mesh_load_from_obj: failed to open file %s", filename);
		return NULL;
	}
	...
	while (fscanf(file, "%s", buf) != EOF)
	{
		...
		case 'v':
			...
			case 'n':
				...
				break;
			case 't':
				fscanf(file, "%f %f %f", &mesh->texels[t].x, &mesh->texels[t].y);
				t++;
				break;
			default:
				break;
